...
# Columns added after the initial schema. Each ALTER is expected to fail once the column
# exists, so these run silently (a failure here is the normal "already applied" case).
_COLUMN_ADDITIONS = [
...
async def _add_new_columns(db) -> None:
    for sql in _COLUMN_ADDITIONS:
        try:
...
async def _migrate_reorganize_queue(db) -> None:
    """Migrate deorganize_queue → reorganize_queue (adds the direction column)."""
    has_deorg = bool(
        await (
            await db.execute(
...
async def _ensure_repo_files_table(db) -> None:
    """Create repo_files for installations predating F-39 (fresh installs have it from _SCHEMA)."""
    await db.execute(
        "CREATE TABLE IF NOT EXISTS repo_files ("
        "  id INTEGER PRIMARY KEY AUTOINCREMENT,"
        "  model_type TEXT NOT NULL,"
...
async def _ensure_catalog_entries_table(db) -> None:
    """Create catalog_entries for installs predating F-40 (fresh installs have it from _SCHEMA)."""
    await db.execute(
        "CREATE TABLE IF NOT EXISTS catalog_entries ("
        "  id INTEGER PRIMARY KEY AUTOINCREMENT,"
        "  source_platform TEXT NOT NULL,"
...
async def _migrate_f40_catalog(db) -> None:
    """Seed catalog_entries from existing models, link models, and re-key repo_files."""
    await db.execute(
...
async def _ensure_download_history_table(db) -> None:
    """Create download_history for installs predating F-47 (fresh installs have it from _SCHEMA)."""
    await db.execute(
        "CREATE TABLE IF NOT EXISTS download_history ("
        "  id INTEGER PRIMARY KEY AUTOINCREMENT,"
        "  model_name TEXT NOT NULL,"
...
# Ordered migration steps run after the column additions. The catalog_entries table and the
# catalog_entry_id column must exist before _migrate_f40_catalog seeds and links them, so the
# order here matters. Each step is isolated: a failure is logged and the remaining steps run.
_MIGRATION_STEPS = (
    ("tag schema", _migrate_tags_schema),
    ("reorganize_queue", _migrate_reorganize_queue),
    ("repo_files", _ensure_repo_files_table),
    ("catalog_entries", _ensure_catalog_entries_table),
    ("F-40 catalog", _migrate_f40_catalog),
    ("download_history", _ensure_download_history_table),
    ("filename_keywords", _migrate_filename_keywords),
)


async def _migrate_db():
    """Apply post-schema migrations; safe to re-run (each step is idempotent and isolated)."""
    async with aiosqlite.connect(cfg.db_path()) as db:
        await _add_new_columns(db)
        for name, step in _MIGRATION_STEPS:
            try:
                await step(db)
            except Exception:
                # Isolate failures so one broken step can't block the rest; surface it in
                # logs rather than swallowing silently (a silent pass once hid a real bug).
                _log.warning("Migration step %r failed", name, exc_info=True)
        await db.commit()
