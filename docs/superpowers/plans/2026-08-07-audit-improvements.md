...
- [ ] **Step 1:** `git branch -D images-recreate` (its content is merged as squash commit `8cb733c`).
- [ ] **Step 2:** From `frontend/`: `npm update` → pulls `@angular/build`/`@angular/cli` 21.2.20, `dompurify` 3.4.13, `typescript-eslint` 8.66.0 (all in-range).
- [ ] **Step 3:** `npm audit` — expect the 3 undici findings to remain (fixed only by Angular 22, Task 10). Record output.
- [ ] **Step 4:** Full frontend gate run (test, lint, format:check, build).
- [ ] **Step 5:** Commit `chore(deps): npm update to latest in-range patches`.
...
- [ ] **Step 1:** Sweep with `grep -rn '@pytest.fixture()' tests/ conftest.py` (and any other zero-arg decorator Sonar flagged) and strip the parens at the 17 flagged sites.
- [ ] **Step 2:** Backend gate run (pytest + ruff check + ruff format --check).
- [ ] **Step 3:** Commit `test: drop empty decorator parentheses (SonarQube S9083)`.
...
- [ ] **Step 1:** Read the four call sites via Serena; add the wrapper coroutines next to the existing `_hash_file`-style seams.
- [ ] **Step 2:** Replace the direct calls with `await <wrapper>(...)`.
- [ ] **Step 3:** Existing route tests must still pass unchanged (they exercise the same handlers); where a test monkeypatches the old sync path, patch the new wrapper instead.
- [ ] **Step 4:** Backend gate run.
- [ ] **Step 5:** Commit `perf: run blocking file IO through asyncio.to_thread in route handlers`.
...
- [ ] **Step 1:** Write/extend a regression test asserting the exact current output shape for 3 models with trigger words, media, and tags (run against current impl → green baseline).
- [ ] **Step 2:** Rewrite hydration: one `SELECT ... WHERE model_id IN (…)` per child table (trigger_words, media, tags), group rows by `model_id` in Python, stitch into the parent dicts. 1+3N queries → 4.
- [ ] **Step 3:** Re-run the regression test + full backend gate.
- [ ] **Step 4:** Commit `perf: batch child-table hydration in get_metadata_by_filenames`.
...
- [ ] **Step 1:** Replace each `except Exception: pass` with logging via the module's existing logger (add `logger = logging.getLogger(__name__)` if absent), e.g. `logger.warning("reorganize failed for %s", path, exc_info=True)`. Behavior (fallback path) unchanged. `py/video_poster.py` stays as-is (deliberate fallback chain).
- [ ] **Step 2:** Backend gate run.
- [ ] **Step 3:** Commit `fix: log swallowed exceptions in reorganize/media-cleanup paths`.
...
- [ ] **Step 1:** Sweep each file: `grep -n '\.subscribe(' frontend/src/app` → for each site not already guarded, add `takeUntilDestroyed(this.destroyRef)`. Rationale: one-shot HTTP observables complete anyway; the guard prevents late callbacks writing to destroyed components.
- [ ] **Step 2:** Modernize `backend-notification.ts` (remove manual `ngOnDestroy` unsubscribe).
- [ ] **Step 3:** Full frontend gate run (test, lint, format:check, build).
- [ ] **Step 4:** Commit `refactor: guard all subscriptions with takeUntilDestroyed`.
- [ ] **Step 5:** Append the convention to `mem:conventions` (Serena) and commit the memory file immediately (project rule).
...
- [ ] **Step 1:** Write the spec first: create real `<img>` elements, dispatch synthetic events, assert `style.display` transitions and the `previousElementSibling` ▶-hiding contract (per `mem:conventions` video-poster pattern); assert `videoPosterUrl` encoding.
- [ ] **Step 2:** Run spec → fails (module missing).
- [ ] **Step 3:** Implement `media-events.ts`; components keep thin methods (`onImgLoad(e) { showOnLoad(e); }`) so templates don't change.
- [ ] **Step 4:** Delete the 6 duplicated bodies; `npx prettier --write` the new files.
- [ ] **Step 5:** Full frontend gate run; commit `refactor: extract shared media load/error/poster helpers`.
...
- [ ] **Step 1:** Add translation keys (e.g. `common.manually_edited`, `common.click_to_toggle`, `common.click_to_add`, `common.delete`, `models.confirm_delete_bulk` with `{count}` param, `model_detail.base_model_placeholder`) to every locale file present.
- [ ] **Step 2:** Replace hardcoded `title="..."`/`placeholder="..."` with `[title]="'key' | translate"` / `[placeholder]="'key' | translate"`; add `[attr.aria-label]` mirroring the title on the icon-only buttons (×, delete) per the form-control-labeling convention.
- [ ] **Step 3:** Replace the native `confirm()` in `models.ts:353` with the existing `app-confirm-popover` component (same pattern as single-delete).
- [ ] **Step 4:** Update/extend specs where they assert on the old strings; full frontend gate run.
- [ ] **Step 5:** Commit `fix: translate hardcoded UI strings and add aria-labels (i18n/a11y)`.
...
- [ ] **Step 1:** Convert each eager `component:` to `loadComponent:`; keep redirects (incl. the `RedirectFunction` for `catalog/:platform`) untouched.
- [ ] **Step 2:** Fix the route-spec stub mapper to also stub `loadComponent` routes.
- [ ] **Step 3:** `npx ng build` — verify the initial bundle shrinks; if comfortably under 650 kB, lower `maximumWarning` in `angular.json` back to 650 kB.
- [ ] **Step 4:** Full frontend gate run; commit `perf: lazy-load all page routes`.
...
- [ ] **Step 1:** `npx ng update @angular/core@22 @angular/cli@22`, then `npx ng update @angular-eslint/schematics@22` (angular-eslint 22 for ESLint compat); bump `typescript-eslint` only if peer ranges require it.
- [ ] **Step 2:** Re-check the two standing gotchas from `mem:conventions`: whether the `@hono/node-server` override is still needed, and that the `frontend/js` asset-path rule still holds under the v22 builder.
- [ ] **Step 3:** `npm audit` — verify the undici findings are gone.
- [ ] **Step 4:** Full frontend gate run incl. build; fix any migration fallout (API renames, builder options).
- [ ] **Step 5:** Commit `chore(deps): upgrade to Angular 22.1 (fixes undici advisory)`; update `mem:tech_stack` (Angular version) and commit the memory immediately.
...
- [ ] **Step 1:** Add the three families; run `ruff check` → triage. Auto-fix with `--fix` where safe; hand-fix the rest. Add per-rule ignores only with a documented reason (e.g. mutable class attributes in ComfyUI node definitions if RUF012 fires on them).
- [ ] **Step 2:** Backend gate run (pytest + format check still clean).
- [ ] **Step 3:** Commit `chore: enable ruff A/C4/RUF rule families`; note the new families in `mem:tech_stack` and commit the memory immediately.
...
- [ ] **Step 1:** Backend: for each target, list uncovered lines (`pytest --cov=py --cov-report=term-missing`), write unit tests for the biggest contiguous gaps (nodes utils, notifications routes, static path guards, metadata_fetcher error paths, civitai provider parsing branches).
- [ ] **Step 2:** Frontend: one spec per untested service using `provideHttpClientTesting` + `expectOne(...).flush(...)` per `mem:conventions` (protects the ≥62% function-coverage gate).
- [ ] **Step 3:** Full gates both sides; confirm backend stays ≥88% and improves.
- [ ] **Step 4:** Commit `test: lift coverage on nodes utils, notification/static routes, providers, frontend services`.
